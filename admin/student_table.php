...
<body style="font-family: Arial, sans-serif; background-color: #f5f5f5; padding: 30px;">
    ...
    <div
        ...
        <h2 style="text-align: center; margin-bottom: 20px;">Students Registry</h2>
        ...
        <table style="width: 100%; border-collapse: collapse;">
            ...
            <tbody>
                <?php
                // Select only students from the database
                $papar = mysqli_query($conn, "SELECT * FROM user WHERE status = 'student' ORDER BY nama ASC");
                ...
                while ($row = mysqli_fetch_array($papar)) {
                    ...
                    echo "<td style='padding: 10px; border: 1px solid #ddd; text-align: center;'>";
                    echo "<a href='delete.php?no_kp=" . htmlspecialchars($row['no_kp']) . "' onclick=\"return confirm(' Are you sure you want to delete this student record? This action cannot be undone.')\"  color: white; padding: 6px 12px; border-radius: 6px; font-size: 0.9em;'>Delete</a>";
                    echo "</td>";
    ...
    <script>
        ...
        function downloadBarcodeJS(barcodeFileName, studentName) {
            if (!barcodeFileName) {
                alert('No barcode file available for this student.');
                return;
            }
            ...
            testAndDownload(possiblePaths, 0, studentName, barcodeFileName);
        }

        function testAndDownload(paths, index, studentName, fileName) {
            ...
            img.onerror = function () {
                // Try next path
                testAndDownload(paths, index + 1, studentName, fileName);
            };
            img.src = paths[index];
        }
