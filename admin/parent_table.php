...
<body style="font-family: Arial, sans-serif; background-color: #f5f5f5; padding: 30px;">
    ...
    <div
        ...
        <h2 style="text-align: center; margin-bottom: 20px;">Parents Registry</h2>
        ...
        <table style="width: 100%; border-collapse: collapse;">
            ...
            <tbody>
                <?php
                $papar = mysqli_query($conn, "SELECT * FROM user WHERE status = 'parent' ORDER BY nama ASC");
                ...
                while ($row = mysqli_fetch_array($papar)) {
                    ...
                    echo "<td style='padding: 10px; border: 1px solid #ddd;'><span style='color: black; padding: 4px 12px; border-radius: 12px; font-size: 0.9em;'>" . htmlspecialchars($row['status']) . "</span></td>";
                    ...
                    echo "<td style='padding: 10px; border: 1px solid #ddd; text-align: center;'>";
                    echo "<a href='delete.php?no_kp=" . htmlspecialchars($row['no_kp']) . "' onclick=\"return confirm(' Are you sure you want to delete this parent record? This action cannot be undone.')\"  color: white; padding: 6px 12px; border-radius: 6px; font-size: 0.9em;'>Delete</a>";
                    echo "</td>";
    ...
    <script>
        ...
        function downloadBarcodeJS(barcodeFileName, parentName) {
            if (!barcodeFileName) {
                alert('No barcode file available for this parent.');
                return;
            }
            ...
            testAndDownload(possiblePaths, 0, parentName, barcodeFileName);
        }

        function testAndDownload(paths, index, parentName, fileName) {
            ...
            img.onload = function () {
                ...
                const cleanName = parentName.replace(/[^a-zA-Z0-9]/g, '_');
            ...
            img.onerror = function () {
                // Try next path
                testAndDownload(paths, index + 1, parentName, fileName);
            };
            img.src = paths[index];
        }
